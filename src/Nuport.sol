...
/// @title Nuport: Nubit -> EVM, Data Availability relay.
/// @dev The relay relies on a set of signers to attest to some event on
/// Nubit. These signers are the Nubit validator set, who sign over every
/// Nubit block. Keeping track of the Nubit validator set is accomplished
...
/// in a Nubit block header. Relayed tuples are in the same order as the
/// block headers.
...
contract Nuport is IDAOracle, Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    /// @notice Relays a root of Nubit data root tuples to an EVM chain. Anyone
    /// can call this function, but they must supply valid signatures of the
    ...
    /// on Nubit, _in order of inclusion_.
    ///
