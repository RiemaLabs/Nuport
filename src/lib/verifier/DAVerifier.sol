...
/// the Nubit network, were committed to by the Nuport smart contract.
struct SharesProof {
    ...
    // The proof of the data root tuple to the data root tuple root that was posted to the Nuport contract.
    AttestationProof attestationProof;
}

/// @notice Contains the necessary parameters needed to verify that a data root tuple
/// was committed to, by the Nuport smart contract, at some specif nonce.
...
/// @title DAVerifier: Nubit -> EVM, Data Availability verifier.
/// @dev The DAVerifier verifies that some shares, which were posted on Nubit, were committed to
/// by the Nuport smart contract.
library DAVerifier {
    /////////////////
    // Error codes //
    /////////////////
    ...
    /// @notice Verifies that the shares, which were posted to Nubit, were committed to by the Nuport smart contract.
    /// @param _bridge The Nuport smart contract instance.
    ...
    {
        // checking that the data root was committed to by the Nuport smart contract.
    ...
    /// NOTE: This doesn't authenticate the proof to Nuport. It only verifies if the proof is valid.
    ...
    /// @notice Verifies that a row/column root, from a Nubit block, was committed to by the Nuport smart contract.
    /// @param _bridge The Nuport smart contract instance.
    ...
    /// @notice Verifies that a row/column root proof, from a Nubit block, to the data root tuple root.
    /// NOTE: This doesn't authenticate the proof to Nuport. It only verifies if the proof is valid.
    /// @param _rowRoot The row/column root to be proven.
    /// @param _rowProof The proof of the row/column root to the data root.
    ...
    /// @notice Verifies that a set of rows/columns, from a Nubit block, were committed to by the Nuport smart contract.
    /// @param _bridge The Nuport smart contract instance.
    ...
    /// @notice Verifies the proof a set of rows/columns, from a Nubit block, to their corresponding data root.
    /// NOTE: This doesn't authenticate the proof to Nuport. It only verifies if the proof is valid.
    /// @param _rowRoots The set of row/column roots to be proved.
    /// @param _rowProofs The set of proofs of the _rowRoots in the same order.
    ...
    /// @notice computes the Nubit block square size from a row/column root to data root binary merkle proof.
    /// The square size is the number of rows of the original square.
    /// Note: the provided proof is not authenticated to the Nuport smart contract. It is the user's responsibility
    ...
    /// @notice computes the Nubit block square size from a shares to row/column root proof.
    /// The square size is the number of rows of the original square.
    /// Note: the provided proof is not authenticated to the Nuport smart contract. It is the user's responsibility
